	...
	function moveSingleElement()
	{
		...
		for(var no=0;tmpTextArray.length<500&&no<tmpTextArray.length;no++){
			var items = tmpTextArray[no].split('___');
			tmpFromBox.options[no] = new Option(items[0],items[1]);
			
		}		
		
		for(var no=0;tmpTextArray2.length<500&&no<tmpTextArray2.length;no++){
	...
	function selectMultiStaff(fieldId, divId, tagId) {
	    var selectedStaffs = encodeURIComponent($(tagId).value.trim());
	
	    var returnValue = showModalDialog('userManageAction.it?method=enterSelectStaff&selectedStaffs='+selectedStaffs,
                   window,'dialogWidth:475px; dialogHeight:500px;help:0;status:0;resizable:1;');
        if (returnValue != null){
          document.all[tagId].value = returnValue;    
          //document.all[divId].innerText = returnValue;          
        }
	}