...
setCAValue();
  ...
  if(document.getElementsByName("subBtn")[0]) {
            $("subBtn").onmouseover=function(){changeAmount("02_11","04_2");}
  }
  if(document.getElementsByName("addBtn")[0]) {
            $("addBtn").onmouseover=function(){changeAmount("02_11","04_2");}
  }
...
function setCAValue(){
                       try{
   var sql="SELECT field_04_6  V1 from teflow_89_04 where request_no='"+$("request_no").value+"' ";
   var savedCA="";   
...
var sql = "select a.request_no %2B '     |' %2B c.field_03_1 %2B '|' %2B str(c.field_03_3,10,2) V1,a.request_no V2"+
...
function onchangeRequester(staffCode) {
  ...
  if(xmlObj){
    ...
    if(options && options.length){
      ...
      $("field_01_6").value= options[0].getElementsByTagName("OPTION_VALUE2")[0].childNodes[0].nodeValue;
      document.getElementsByName("field_01_6")[0].style.color =cn_name_color;
    }
  }
...
function rowTotal(ob){
	...
	var exrate = 1.000;
	var total;
                     if(document.getElementsByName("field_02_7")[i].value.replace(/,/g,'') != ''){ 
	   v7=0+parseFloat(document.getElementsByName("field_02_7")[i].value);
                    }
                     if(document.getElementsByName("field_02_8")[i].value.replace(/,/g,'') != ''){ 
                         v7=v7+0+parseFloat(document.getElementsByName("field_02_8")[i].value);
                     }
	exrate=document.getElementsByName("field_02_10")[i].value;

	total=parseFloat(v7)*exrate;
	document.getElementsByName("field_02_11")[i].value=formatNumber(total,'#,##0.00');
	changeAmount("02_11","04_2");
}
...
function changeSP(ob){
  if(ob.value>0){
    document.getElementsByName("field_02_9")[ob.rowIndex].required="true";
  }else{
    document.getElementsByName("field_02_9")[ob.rowIndex].required="false";
  }
}
...
afterSetDay = function(obj){
    ...
    changeCurrencyByDate('field_02_6','field_02_10',obj);
    rowTotal(obj);
    afterSetDayFlag = 0;
	}
}
