...
//**************************************************** 
/////////////////////////////////////////////////////////////////////////////////
var INF_IS_IEXPLORER = (navigator.userAgent.indexOf("MSIE") == -1) ? false
		: true;
...
var CFG_DOCUMENT_MARGIN = 2;
var CFG_MENU_SPACING = 0;
var CFG_MENU_WIDTH = 175;
var CFG_SCROLLBAR_WIDTH = 17;
var CFG_TITLEBAR_HEIGHT = 38;
var CFG_TITLEBAR_LEFT_WIDTH = 0;
var CFG_TITLEBAR_RIGHT_WIDTH = 25;
var CFG_TITLEBAR_MIDDLE_WIDTH = CFG_MENU_WIDTH - CFG_TITLEBAR_LEFT_WIDTH
		- CFG_TITLEBAR_RIGHT_WIDTH;
...
if (new Array().splice == null) {
	...
	Array.prototype.splice = function() {
		...
		var startPoint2 = (endPoint1 < 0) ? (this.length + endPoint1 + arguments[1])
				: (endPoint1 + arguments[1]);
...
function TaskMenuItem(strLabel, strIcon, strCommand) {
	...
	this.init = function() {
		...
		with (iconCell = itemRow.insertCell(1)) {
			...
			with (this.hIcon = iconCell.appendChild(document
					.createElement("DIV"))) {
	...
	this.setCommand = function(strCommand) {
		if ((this.command = strCommand) != null) {
			JS.addListener(this.hLabel, "mouseover",
					TaskMenuItem.onMouseBehavior, false);
			JS.addListener(this.hLabel, "mouseout",
					TaskMenuItem.onMouseBehavior, false);
			JS.addListener(this.hLabel, "click", TaskMenuItem.onClick, false);
			this.hLabel.className = "MenuItemLabel";
		} else {
			JS.removeListener(this.hLabel, "mouseover",
					TaskMenuItem.onMouseBehavior, false);
			JS.removeListener(this.hLabel, "mouseout",
					TaskMenuItem.onMouseBehavior, false);
			JS.removeListener(this.hLabel, "click", TaskMenuItem.onClick,
							false);
...
function TaskMenu(strLabel, menuState, strImgURL) {	
	this.menuID;
	this.label = strLabel;
	...
	this._getMenuHeight = function() {
		if (INF_IS_IEXPLORER)
			...
			return scrollHeight + CFG_MENUBODY_BORDER_WIDTH + 2
					* CFG_MENUBODY_PADDING;
		}
	}
	...
	this._fixMenuWidth = function(value) {
		var titlebarWidth = (CFG_MENU_WIDTH + ((INF_IS_IEXPLORER) ? 0
				: (-2 * CFG_TITLEBAR_BORDER_WIDTH)));
		var menuBodyWidth = (CFG_MENU_WIDTH + ((INF_IS_IEXPLORER) ? 0
				: (-2 * CFG_MENUBODY_BORDER_WIDTH)));
		var labelCell = this.titlebar.firstChild.rows[0].cells[1];
		var scrollbarWidth = this._getScrollWidth();

		this.hLabel.style.width = (CFG_TITLEBAR_MIDDLE_WIDTH - scrollbarWidth)
				+ "px";
		labelCell.style.width = (CFG_TITLEBAR_MIDDLE_WIDTH - scrollbarWidth)
				+ "px";
	...
	this._fixedMenuPosWhileClose = function() {
		...
		this.menuBody.style.top = (this.titlebar.offsetTop
				+ CFG_TITLEBAR_HEIGHT - scrollHeight)
				+ "px";
		this.menuBody.style.clip = "rect(" + scrollHeight + " auto "
				+ scrollHeight + " auto)";
	...
	this._extend = function() {
		if (this.state & TaskMenuState.close) {
			...
			if (this.menuBody.offsetHeight + this.transformStep < this
					._getMenuHeight()) {
				this._setMenuHeight(this.menuBody.offsetHeight
						+ this.transformStep);
				this.transformTimer = setTimeout("TaskMenu.collection["
						+ this.menuID + "]._extend()", this.transformSleep += 5);
			} else {
		...
		if (this.state & TaskMenuState.close) {
			...
			if (this.menuBody.offsetHeight + this.transformStep > this
				...
				this._setMenuHeight(this.menuBody.offsetHeight
						+ this.transformStep);
				this.transformTimer = setTimeout("TaskMenu.collection["
						+ this.menuID + "]._shorten()",
						this.transformSleep += 5);
			} else {
				this._setMenuHeight(this._getMenuHeight());
				this._removeState(TaskMenuState.transforming);
			}
	...
	this._appendItems = function() {
		if (this.menuBody == null || this.items.length == 0)
			return;
	...
	this._open = function() {
		...
		if ((this.menuBody.offsetTop + this.operationStep) < titlebarBottom) {
			this.menuBody.style.top = (this.menuBody.offsetTop + this.operationStep)
					+ "px";
			this.menuBody.style.clip = "rect("
					+ (titlebarBottom - this.menuBody.offsetTop) + " auto "
					+ (this.menuBody.offsetHeight - 1) + " auto)";
			this._setAlphaOpacity(this._getAlphaOpacity() + CFG_ALPHA_STEP);
			this.operationTimer = setTimeout("TaskMenu.collection["
					+ this.menuID + "]._open()", this.operationSleep);
	...
	this._close = function() {
		...
		if ((menuBodyBottom - this.operationStep) > titlebarBottom) {
			...
			this._setAlphaOpacity(this._getAlphaOpacity() - CFG_ALPHA_STEP);
			this.operationTimer = setTimeout("TaskMenu.collection["
					+ this.menuID + "]._close()", this.operationSleep);
			this.operationSleep += 5;
		} else {
			this.menuBody.style.top = ((titlebarBottom - menuBodyHeight))
					+ "px";
			this._setAlphaOpacity(0);
			this._attachState(TaskMenuState.close);
		}
		this.menuBody.style.clip = "rect("
				+ (Math.abs(titlebarBottom - this.menuBody.offsetTop))
				+ " auto " + (this.menuBody.offsetHeight - 1) + " auto)";
	...
	this._moveTo = function() {
		...
		var safePoint = this.previous.titlebar.offsetTop + CFG_TITLEBAR_HEIGHT
				+ CFG_MENU_SPACING;
		var dstiTop = this.previous.menuBody.offsetTop
				+ this.previous.menuBody.offsetHeight + CFG_MENU_SPACING;
		...
		if (this.state & TaskMenuState.open)
			this.menuBody.style.top = (this.titlebar.offsetTop + CFG_TITLEBAR_HEIGHT)
					+ "px";

		else if (this.state & TaskMenuState.close)
			this.menuBody.style.top = (this.titlebar.offsetTop
					+ CFG_TITLEBAR_HEIGHT - this.menuBody.offsetHeight)
					+ "px";

		else if (this.state & TaskMenuState.opening || this.state
				& TaskMenuState.closing)
			this.menuBody.style.top = (this.menuBody.offsetTop + (dstiTop - primTop))
					+ "px";
	...
	this.init = function(initFlag) {
		if (initFlag != true && (this.titlebar != null || this.menuBody != null))
			return;

		this.state = (this.state == false) ? TaskMenuState.close
				: TaskMenuState.open;
		this.label = (this.label == null) ? new String() : this.label
				.toString();
		...
		with (this.titlebar = document.createElement("DIV")) {
			...
			style.top = ((TaskMenu.headNode == null) ? CFG_DOCUMENT_MARGIN
					: (TaskMenu.rearNode.menuBody.offsetTop
							+ TaskMenu.rearNode.menuBody.offsetHeight + CFG_MENU_SPACING))
					+ "px";
			style.left = CFG_DOCUMENT_MARGIN + "px";
			style.width = (CFG_MENU_WIDTH
					+ ((INF_IS_IEXPLORER) ? 0
							: (-2 * CFG_TITLEBAR_BORDER_WIDTH)) - this
					._getScrollWidth())
					+ "px";
		...
		if (TaskMenu.headNode != null) {
			...
			JS.addListener(titlebarRow, "mouseover", TaskMenu._onMouseBehavior,
					false);
			JS.addListener(titlebarRow, "mouseout", TaskMenu._onMouseBehavior,
					false);
		}
		...
		with (middleCell = titlebarRow.insertCell(1)) {
			...
			style.width = (CFG_TITLEBAR_MIDDLE_WIDTH - this._getScrollWidth())
					+ "px";
			with (this.hLabel = appendChild(document.createElement("DIV"))) {
				className = "MenuTitle" + fixString;
				style.width = (CFG_TITLEBAR_MIDDLE_WIDTH - this
						._getScrollWidth())
						+ "px";
				style.height = (CFG_TITLEBAR_HEIGHT) + "px";
		...
		with (this.hArrayCell = titlebarRow.insertCell(2)) {
			className = "MenuTitlebarRight"
					+ ((this.state == TaskMenuState.close) ? "_Close" : "_Open")
					+ fixString;
		...
		with (this.menuBody = document.createElement("DIV")) {
			...
			if (TaskMenu.headNode != null) {
				...
				style.cssText = "border-width:0px "
						+ CFG_MENUBODY_BORDER_WIDTH
						+ "px "
						+ CFG_MENUBODY_BORDER_WIDTH
						+ "px "
						+ CFG_MENUBODY_BORDER_WIDTH
						+ "px; "
						+ ((this.background != null) ? "background-image:url('"
								+ this.background + "');" : "");
				style.top = (parseInt(this.titlebar.style.top) + CFG_TITLEBAR_HEIGHT) + "px";
				style.left = CFG_DOCUMENT_MARGIN + "px";
				style.width = (CFG_MENU_WIDTH
						+ ((INF_IS_IEXPLORER) ? 0
								: -(2 * CFG_MENUBODY_BORDER_WIDTH)) - ((INF_IS_SCROLLBAR_ENABLED) ? CFG_SCROLLBAR_WIDTH
						: 0))
						+ "px";
				style.height = "auto";
				...
				className = "MenuBodyInital";
				
				style.cssText = "border-width:0px 0px 0px 0px;"
					+ ((this.background != null) ? "background-image:url('"
							+ this.background + "');" : "");
				style.top = parseInt(CFG_TITLEBAR_HEIGHT) + "px";
		...
		if (taskMenuItems instanceof Array) {
			for ( var i = 0; i < taskMenuItems.length; i++) {
				if (taskMenuItems[i] instanceof TaskMenuItem) {
					(dstItem = (taskMenuItems[i].container == null) ? taskMenuItems[i]
							: taskMenuItems[i].cloneNode()).container = this;
					nItem.push(dstItem);
				}
			}
		} else if (taskMenuItems instanceof TaskMenuItem) {
			(nItem[0] = (taskMenuItems.container == null) ? taskMenuItems
					: taskMenuItems.cloneNode()).container = this;
		} else
			return;

		if (iPosition == null || typeof iPosition != "number" || iPosition < 0
				|| iPosition > nItem.Length) {
			iPosition = this.items.length;
		}
		this.items = this.items.slice(0, iPosition).concat(nItem,
				this.items.slice(iPosition));
	...
	this.setLabel = function(label) {
		...
		/*with (this.hLabel) {
			(INF_IS_IEXPLORER) ? innerText = this.label
					: textContent = this.label;
		}*/
		this.hLabel.innerHTML =  "<img src=\""+this.imgURL+"\" class=\"menuImg\" >" + this.label + "";
		this.titlebar.title = this.label;
	}
	
	this.setLabel2 = function(label) {
		this.label = "<img src=\""+this.imgURL+"\" class=\"menuImg\" >" +
				"<a href=\"#\" onclick=\"goHomePage()\" " +
					"style=\"text-decoration: none;color:#ffffff;\">"+label.toString()+ "</a>";
			//"<a href=\"/eflowCS\" style=\"text-decoration: none;color:#ffffff;\">"+label.toString()+ "</a>";
		
		this.hLabel.innerHTML = this.label;
	}
	...
	this.invalidate = function() {
		...
		this.transformStep = (tfDistance > 0) ? Math.ceil(tfDistance
				/ CFG_FRAME_COUNT) : Math.floor(tfDistance / CFG_FRAME_COUNT);

		if (this.state & TaskMenuState.closing)
			this.operationStep = Math.ceil(nScrollHeight / CFG_FRAME_COUNT);

		if (tfDistance > 0)
			this.invalidateTimer = setTimeout("TaskMenu.collection["
					+ this.menuID + "]._extend()", 50);

		else
			this.invalidateTimer = setTimeout("TaskMenu.collection["
					+ this.menuID + "]._shorten()", 50);
	}
	this.click = function() {
		...
		if (this.state & TaskMenuState.open || this.state
				& TaskMenuState.opening) {
			this._attachState(TaskMenuState.closing);
			this.hArrayCell.className = "MenuTitlebarRight_Close_MouseOver"
					+ nodeTypeFix;
			this._close();
		} else if (this.state & TaskMenuState.close || this.state
				& TaskMenuState.closing) {
			this._attachState(TaskMenuState.opening);
			this.hArrayCell.className = "MenuTitlebarRight_Open_MouseOver"
					+ nodeTypeFix;
			this._open();
		}
	}
...
TaskMenu._onMouseBehavior = function() {
	...
	if(refMenu!=undefined)
	{
		...
		if ((refMenu.state & TaskMenuState.open)
				|| (refMenu.state & TaskMenuState.opening)) {
			refMenu.hArrayCell.className = "MenuTitlebarRight_Open" + behaviorFix
					+ nodeTypeFix;
		} else if ((refMenu.state & TaskMenuState.close)
				|| (refMenu.state & TaskMenuState.closing)) {
			refMenu.hArrayCell.className = "MenuTitlebarRight_Close" + behaviorFix
					+ nodeTypeFix;
		}
	}
}
...
JS.addListener(window, "resize", new Function(
		"setTimeout(\"TaskMenu._refreshAll()\",30);"), false);
JS.addListener(window, "error", new Function("return true"), false);

function goHomePage(){
	parent.window.frames[1].location.href='main.jsp';
}
