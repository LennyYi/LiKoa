...
var defaultBusinessType="";		//20150506 业务渠道	Justin Bin Added
var nMaxAge = 0;
var nMinAge = 0;
var formSystemId = "";

function setRenewal(){
	jQuery('select[name="field_02_33"]').change(function(){
		if(jQuery(this).val() == 'R')
			jQuery('textarea[name="field_10_10"]').val('R');
		else
			jQuery('textarea[name="field_10_10"]').val('F');
	});
}

function addandsubtractDay(){
	jQuery('input[name="field_02_34"]').blur(function(){
		var dayValue = jQuery(this).val();
		if(formSystemId == '30288') 
			jQuery('input[name="field_02_37"]').val(addDate(dayValue,3)).trigger('blur');
		
		if(formSystemId == '30320')
			jQuery('input[name="field_02_37"]').val(addDate(dayValue,5)).trigger('blur');
	});
}
...
function triggerBasicAction(){
	...
	jQuery("textarea[name='field_02_16']").focus(function(){loadPic(this);}).blur(function(){DupChecking_Client(this); });
...
function triggerTableAction(){
	...
	bindTableNewRowAction("formTableProduct", 1);
	bindTableNewRowAction("formTableplanDetail", 1);
}
...
function bindTableNewRowAction(tableId, type){
	...
	switch (tableId) {
		...
		case "formTable9": {
			...
			//NOVA-290
			if(formSystemId !='30322' && formSystemId !='30320' ){
			...
			//if(formSystemId!=="30328")			//if JS_flex form, no need to calculate 
				bindTableTRAction(tableId,'input[name="field_9_9"]','blur','calculatePremium(this,true);',type);
			
			if(formSystemId == '20259' || formSystemId=='30319' || formSystemId=='30328' )
				...
				//bindTableTRAction(tableId,'select[name="MemProduct_id"]','focus','loadMemPkgPdt(this);',type);
				bindTableTRAction(tableId,'select[name="MemProduct_id"]','focus','loadMemProduct(this,false,true,false);',type);
		...
		//20150507 Justin Bin Added
		case "formTableProduct":{
			//bindTableTRAction(tableId,'select[name="field_Product_1"]','blur','showProduct(this);',type);
			if(formSystemId=="30321")
				bindTableTRAction(tableId,'select[name="field_Product_1"]','blur','showProduct(this);',type);
			bindTableTRAction(tableId,'select[name="field_Product_1"]','blur','showProductPlan();',type);
			bindTableTRAction(tableId,'select[name="field_Product_1"]','change','calSumFun();',type);
			bindTableTRAction(tableId,'select[name="field_Product_2"]','change','calSumFun();',type);
			
			bindTableTRAction(tableId,'textarea[name="field_Product_3"]','blur','AddPrecentageSign(this);',type);
			bindTableTRAction(tableId,'textarea[name="field_Product_4"]','blur','AddPrecentageSign(this);',type);
			bindTableTRAction(tableId,'textarea[name="field_Product_5"]','blur','AddPrecentageSign(this);',type);
			bindTableTRAction(tableId,'textarea[name="field_Product_6"]','blur','AddPrecentageSign(this);',type);
			bindTableTRAction(tableId,'textarea[name="field_Product_7"]','blur','AddPrecentageSign(this);',type);
			bindTableTRAction(tableId,'textarea[name="field_Product_8"]','blur','AddPrecentageSign(this);',type);
			bindTableTRAction(tableId,'textarea[name="field_Product_9"]','blur','AddPrecentageSign(this);',type);
			break;
			
		}
		
		case "formTableplanDetail":{
			bindTableTRAction(tableId, 'input[name="field_planDetail_3"]','blur','calSumFun();',type);
			bindTableTRAction(tableId, 'input[name="field_planDetail_4"]','blur','calSumFun();',type);
		}
	}
    ...
    if(nClntCode != undefined )
    {
        ...
        //alert (sql);
          if (data.length>0) {
                ...
                jQuery("input#combox_transform_OriginalPol").outerWidth(100);
                jQuery("input#combox_transform_OriginalPol").parent().outerWidth(120);
          }
...
function initPendSectionBtn(){
	
	var delBtn = jQuery('#formTablePend').parent().find('input[name="deleteRowBtn"]');
	
	if (delBtn) {
		var button = ' <input type="button" id="impt_formTablePend" value="照会检查" onclick="SubmitValidation()" /> ';
		var copyBut = ' <input type="button" name="copy_formTablePend" id="copy_formTablePend" value="CopyAll" onclick="copyAll();getSystemId4Pending ()"/> ';
		delBtn.after(button);
...
function copyAll(){
	...
	//这里无法固定，字符流与现实文本中的字数是不相同的。这里固定为文本中全部为汉字的宽度
	var colNum =41;
...
function initMemSectionBtn() {
	var tid = "'formTable9'";
	
	var delBtn = jQuery('#formTable9').parent().find('input[name="deleteRowBtn"]');
		...
		jQuery.each(trLast.find('td'),function(i1,item1){
			...
			switch (tagName) {
				...
				case "SELECT": {
					...
					child.combobox("setValue",preChild.val());
...
function appendHiddenToSubOffic(object, value, fieldName, type){
	if(type=='select'){
		object.parent().find('select[name="'+fieldName+'"]').combobox("setValue",value);
		object.parent().find('select[name="'+fieldName+'"]').combobox("disable",true);
	}
	else
