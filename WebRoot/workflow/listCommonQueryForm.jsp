...
%>
<table width="1025" border="<%=ISEXCEL?"1":"0"%>" cellpadding="0" cellspacing="1"
	class="sortable" id="mytable" style="border:1px #cccccc solid;">
	...
	<tr class="tr_change">
      ...
      <%}%>
		<td><a <%=vo.getHtmlTitleAttr()%>
			href="javascript:openFormWithLayer('<%=request.getContextPath()%>/formManageAction.it?method=displayFormContent&operateType=view&viewFlag=false&status=<%=vo.getStatus()%>&requestNo=<%=vo.getRequestNo()%>&formSystemId=<%=vo.getFormSystemId()%>&openType=sub');"><%=vo.getRequestNo()%>&nbsp;&nbsp;</td>
...
<script language="javascript">
	...
	parent.document.all("iframeName").style.height = document.getElementById("mytable").scrollHeight*1.6;
