...
<%@page
	import="com.aiait.framework.db.*,java.util.*,com.aiait.eflow.formmanage.vo.*,com.aiait.eflow.formmanage.action.*,com.aiait.eflow.common.*,com.aiait.eflow.common.helper.*,com.aiait.eflow.util.FieldUtil"%>
<%@page import="com.aiait.eflow.housekeeping.vo.FormTypeVO,com.aiait.eflow.common.helper.FormTypeHelper,com.aiait.eflow.formmanage.vo.FormManageVO,com.aiait.eflow.common.*,com.aiait.eflow.common.helper.AuthorityHelper,com.aiait.eflow.housekeeping.vo.CompanyVO,com.aiait.eflow.housekeeping.action.ParamConfigAction"%>
...
function loadNodeList(){
	var org_id = jQuery('select[name="companyId"]').val(); 
		
	var sql = "select distinct t3.node_name as NODENAME from teflow_wkf_define t1";
	sql += " inner join teflow_form t2 on t1.form_system_id = t2.form_system_id and t2.org_id = '"+org_id+"' ";
	sql += " and t2.[status] = '0' inner join teflow_wkf_detail t3 on t1.flow_id = t3.flow_id ";

	var data = selectFromSQLJson('',sql);

	var nodeName = jQuery("select#nodeName");
	nodeName.empty();
	...
	if(data!="" && data!=null && data.length>0)
	{
		if(data.length>0){
		     jQuery.each(data, function(i,item){ 
		    	 nodeName.append('<option value="'+item.NODENAME+'">'+item.NODENAME+'</option>');  
		     });
		}
	}
}
...
function getStaffList(orgId){
  	var url = "<%=request.getContextPath()%>/userManageAction.it?method=getCompanyStaffList";
   	...
   	var myAjax = new Ajax.Request(url, {
          ...
          onComplete:function(x){ updateStaffList(x.responseXML); },
          onError:function(x){ alert('Fail to get staff list for company'); } 
     }); 
   	loadNodeList();
}
  ...
  function verifyCompletedDates(){
   ...
   function searchForm(){
	  document.all['beginSubmissionDate'].value=formatDate1(document.all['beginSubmissionDate2'].value,dateFormat,"MM/dd/yyyy");
	  document.all['endSubmissionDate'].value=formatDate1(document.all['endSubmissionDate2'].value,dateFormat,"MM/dd/yyyy")
	  
      if(document.all['beginSubmissionDate'].value.Trim()!=""){
        if(isDate(document.all['beginSubmissionDate'],"Submitted From")==false){
          return;
        }
      }
      if(document.all['endSubmissionDate'].value.Trim()!=""){
        if(isDate(document.all['endSubmissionDate'],"Submitted To")==false){
          return;
        }
      }
      if(document.all['beginSubmissionDate'].value.Trim()!="" && document.all['endSubmissionDate'].value.Trim()!=""){
         if(compareDate(document.all['beginSubmissionDate'].value.Trim(),document.all['endSubmissionDate'].value.Trim())==false){
            alert(from_to_date_compare);
            document.all['beginSubmissionDate'].focus();
            return;
         }
      }

      if (!(verifyCompletedDates())) return;
      //alert(document.all['beginSubmissionDate'].value);
...
<%
  ...
  if(needquery!=null && "false".equals(needquery)){
	  beginSubmissionDate = (String)request.getAttribute("beginSubmissionDate2");
	  endSubmissionDate = (String)request.getAttribute("endSubmissionDate2");
	  //String nodeNode = (String)request.getAttribute("nodeName");
	  ...
	  //request.setParameter("needquery","true");
  }
...
--></table>
 <TABLE WIDTH="1036" bordercolor="#cccccc" style="border-collapse:collapse;"  BORDER=1 CELLPADDING=3 CELLSPACING=0 class="tr0" >
   <TR align="center" >
   ...
   <TR> 
      ...
      <TD width=35% height="20" > 
      <font size="2"> 
       <INPUT Name="beginSubmissionDate" type="hidden">
       <INPUT Name="beginSubmissionDate2" onclick='setday(this)'Type="text" value="<%=beginSubmissionDate==null?"":beginSubmissionDate%>" class="text2" style="WIDTH: 130px" id="beginSubmissionDate">(<script>document.write(dateFormat);</script>)
      </font>
      </TD>
      <TD width=15% height="20" class="tr3"><div align="right"><span class="style1"><i18n:message key="common.submit_date_to"/></span></div></TD>
      <TD width=35% height="20" ><font size="2"> 
       <INPUT Name="endSubmissionDate" type="hidden">
       <INPUT Name="endSubmissionDate2" onclick='setday(this)'  Type="text" value="<%=endSubmissionDate==null?"":endSubmissionDate%>" class="text2" style="WIDTH: 130px" id="endSubmissionDate">(<script>document.write(dateFormat);</script>)
      </TD>
    </TR>
   <TR> 
      ...
      <TD width=35% height="20" > 
      <font size="2"> 
       <INPUT Name="beginCompletedDate" onclick='setday(this)'  Type="text" value="<%=beginCompletedDate==null?"":beginCompletedDate%>" class="text2" style="WIDTH: 130px" id="beginCompletedDate">(<script>document.write(dateFormat);</script>)
      </font>
      </TD>
      <TD width=15% height="20" class="tr3"><div align="right"><span class="style1"><i18n:message key="common.complete_date_to"/></span></div></TD>
      <TD width=35% height="20" ><font size="2"> 
       <INPUT Name="endCompletedDate" onclick='setday(this)'  Type="text" value="<%=endCompletedDate==null?"":endCompletedDate%>" class="text2" style="WIDTH: 130px" id="endCompletedDate">(<script>document.write(dateFormat);</script>) 
      </TD>
    </TR>    
   <TR> 
      ...
      <TD width=35% height="20">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <i18n:message key="common.node_name"/>: 
 ...
 <table width="1036"  border="0" cellpadding="0" cellspacing="1" class=sortable id=mytable style="border:1px #cccccc solid;">
...
<%--             <td align='center' ><i18n:message key="common.processed_by"/></td> --%>
          ...
          %>
            <tr class="tr_change">
              ...
              <td ><a href="javascript:openFormWithLayer('<%=request.getContextPath()%>/formManageAction.it?method=displayFormContent&operateType=view&viewFlag=false&requestNo=<%=vo.getRequestNo()%>&formSystemId=<%=vo.getFormSystemId()%>');"
...
<%--             <%
            ...
            %> --%>
              ...
              <td ><%=(vo.getSubmissionDateStr()!=null && !"".equals(vo.getSubmissionDateStr()))?StringUtil.getDateStr(cDate,paramCode):""%>&nbsp;&nbsp;</td>
