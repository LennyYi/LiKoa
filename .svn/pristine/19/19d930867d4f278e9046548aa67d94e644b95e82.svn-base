...
function bindCompassPlanProduct(selectedID, bool, isLoad){
	...
	if(selectedVal!="")
	{
		...
		{	
			...
			reSelected2.combobox("setValue",value1);
			
			if(reSelected.length>0)
				//reSelected.val(prodCode);
				reSelected.combobox("setValue",prodCode);
		}
...
function planChange(selectedID){
	...
	tr.find('select[name="field_4_7"]').combobox("setValue","");
	...
	if(selectedVal!="")
	{
		...
		{	
			...
			if(reSelected)
			{
				...
				reSelected.combobox("setValue",BenplnCD);
			}
		}
	}
}
...
function triggerPlanColumn(){
	...
	//jQuery('#formTable12 tr').find('td:contains("¼Æ»®")').hide();
	...
	if(jQuery("#formTable4 tr").length >= 2){
		var planItem = jQuery("#formTable4 select[name='Plan_id'],#formTable4 input[name='Plan_id']");
