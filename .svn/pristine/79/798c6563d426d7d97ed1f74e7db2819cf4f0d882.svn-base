...
import com.aiait.eflow.wkf.vo.FormBasicDataVO;
...
public class ListInquiryFormDAO extends WorkFlowProcessDAO {
  ...
  /**IT0958 : add filter : requested by
     ...
     */
    public Collection exportInquiryForm(WorkFlowProcessVO queryVo, PageVO page, String supperOrgs,String nodeName) throws DAOException {
	...
	protected HashMap makeCondition(WorkFlowProcessVO queryVo, String supperOrgs){
		...
		if(queryVo.getNodeName() !=null && !"".equals(queryVo.getNodeName())){
			 sql.append(" and a.request_no in (select t1.request_no from teflow_wkf_process t1 ");
			 sql.append(" inner join teflow_wkf_detail t2 on t1.node_id = t2.node_id and t1.flow_id = t2.flow_id and t2.node_name = '"+queryVo.getNodeName()+"'");
			 sql.append(" inner join teflow_wkf_define t3 on t1.flow_id = t3.flow_id ");
			 sql.append(" inner join teflow_form t4 on t3.form_system_id = t4.form_system_id and t4.[status] = '0' and t4.org_id ='"+queryVo.getOrgId()+"' )"); 
		}
		...
		if(queryVo.getBeginSubmissionDate()!=null && !"".equals(queryVo.getBeginSubmissionDate())){
			  if(queryVo.getEndSubmissionDate()!=null && !"".equals(queryVo.getEndSubmissionDate())){
				  ...
				  try{
			        parameters[index] = new java.sql.Date(StringUtil.stringToSqlDate(queryVo.getBeginSubmissionDate()+" 00:00:00",paramCode).getTime());
				  }catch(Exception e){}
				  index++;
				  
				  dataType[index] = DataType.DATE;
				  try{
				        parameters[index] = new java.sql.Date(StringUtil.stringToSqlDate(queryVo.getEndSubmissionDate()+" 23:59:59",paramCode).getTime());
					  }catch(Exception e){}
				 index++;
			  }else{
		...
		if (!(StringUtil.isEmptyString(queryVo.getBeginCompleteDate()))){
			...
			dataType[index] = DataType.DATE;
			try{
			     parameters[index] = StringUtil.stringToSqlDate(queryVo.getBeginCompleteDate()+" 00:00:00",paramCode);
			}catch(Exception e){}
			index++;			
		}
		if (!(StringUtil.isEmptyString(queryVo.getEndCompleteDate()))){
			...
			dataType[index] = DataType.DATE;
			try{
				java.sql.Date tdate = StringUtil.stringToSqlDate(queryVo.getEndCompleteDate()+" 00:00:00",paramCode);
