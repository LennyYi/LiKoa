...
<%
    List<PerDiemRateVO2> list = (List<PerDiemRateVO2>) request.getAttribute("rateList");
    String ctxPath = request.getContextPath();
%>
<html>
<head>
<title>Per Diem Rate</title>
...
function addRate() {
    var url = "<%=ctxPath%>/perdiemRateAction2.it?method=editRate&mode=new";
    window.location = url;
};
...
function deleteRate() {
    ...
    if (confirm("Are you sure to delete the selected record(s)?")) {
        document.forms[0].action = "<%=ctxPath%>/perdiemRateAction2.it?method=deleteRate";
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
    ...
    <!--<tr>
        <td><strong><font color='#5980BB'><i18n:message key="housekeeping_perdiemrate.location" /></font></strong></td>
    </tr>
--></table>
<table width="100%" border="0">
    ...
    <tr>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="1" class="sortable" id="mytable"
            ...
            <%
            for (PerDiemRateVO2 rate : list) {
                String id = rate.getId();
            %>
            <tr class="tr_change" align='center'>
                <td><input type="checkbox" name="id" value="<%=id%>"></td>
                <td><%=rate.getEffDate2()%></td>
                <td><a href="<%=ctxPath%>/perdiemRateAction2.it?method=editRate&mode=edit&id=<%=id%>"><%=rate.getCityName()%></a></td>
                <td><%=rate.getGrade()%></td>
                <td><%=rate.getRate2()%></td>
