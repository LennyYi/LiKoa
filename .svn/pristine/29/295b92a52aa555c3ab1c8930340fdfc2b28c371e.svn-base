...
<head>
	...
	<script language="javascript">
	  function addRate() {
	     var url = "<%=request.getContextPath()%>/hotelRateAction.it?method=editRate&editType=new";
	     window.location = url;
	  }
	  ...
	  function deleteRate() {
	    ...
	    if (confirm("Are you sure to delete the selected records")) {
	      document.forms[0].action = "<%=request.getContextPath()%>/hotelRateAction.it?method=deleteRate";
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
 ...
 <!--<tr>
 	<td>
 	  <strong><font color='#5980BB' family='Times New Roman'><i18n:message key="housekeeping_hotelrate.location"/></font></strong>
 	 </td>
 </tr>
--></table>
      ...
      <td>
        <table width="100%"  border="0" cellpadding="0" cellspacing="1" class=sortable id=mytable style="border:1px #8899cc solid;">
          <tr class="liebiao_tou">
            ...
            <td align='center' ><i18n:message key="housekeeping_hotelrate.city"/></td>
            <td align='center' ><i18n:message key="housekeeping_hotelrate.titlegroup"/></td>
            <td align='center' ><i18n:message key="housekeeping_hotelrate.rate"/></td>
          </tr>
          <%
			if (list != null) {
				int i = 1;
				...
				while (it.hasNext()) {
					HotelRateVO vo = (HotelRateVO) it.next();
          %>
            <tr class="tr_change">
              <td align='center' ><input type="checkbox" name="rateId" value="<%=vo.getCity() + "_" + vo.getTitleGroup()%>"></td>
              <td><%=vo.getCity()%>&nbsp;</td>
              <td>
                <a href='<%=request.getContextPath()%>/hotelRateAction.it?method=editRate&editType=edit&rateId=<%=CommonUtil.encoderURL(vo.getCity() + "_" + vo.getTitleGroup())%>'>
