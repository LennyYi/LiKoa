...
function getEachProuctShipOcc(nodeId){
	...
	// get the member product & plan value in section '人员信息'
	...
	for(var i = 1; i<trList.length; i++){
		...
		if (fsPlancode.toString() == pkgPlan.toString() && fsPkgcode.toString() == pkgCode.toString()){
			...
			return [true, o1, o2, o3];
		}
	}		
	return [false, '', '', 0];	
}

function getEachProductAmount(PlanSeq){
	//this paragraph is to get the product benefit amount by the row number of plan selection in section 4.	
	
	// get the table in section '产品计划组合详情'
	var tblData= jQuery('#formTable12')
	var trList = tblData.find("tr");
	var sSeq   = PlanSeq.toString();	
...
function calculatePremiumJQF(nodeId,isSum){
	...
	sShip     = aResultB[2];
	iPlanSeq  = parseInt(aResultB[3]);
	
	
	var aResultA = getEachProductAmount(iPlanSeq);
	bMatchA   = aResultA[0];
...
function calculatePremium(nodeId,isSum){
	...
	{	
		...
		if (systemFormID == '30322'){
			...
			var soverage = "";
			soverage = OverAgeChecking(fnage, systemFormID);	
...
function OverAgeChecking(fnage, systemFormID){
	var CheckingValue = "";
	...
	if(nMaxAge==0||nMaxAge==0)
	{
		...
		if(systemFormID == '30322'){
		  sql = "SELECT MIN(param_value) as minAge,MAX(param_value) as maxAge FROM teflow_param_config where param_code in ('MaxAgeJQFSH','MinAgeJQFSH')"	
		}		
